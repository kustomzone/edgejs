...
struct CompressionHandle {
  ...
  int64_t async_id = -1;
  int32_t provider_type = kUbiProviderZlib;
...
void QueueDestroyIfNeeded(CompressionHandle* handle) {
  ...
  UbiAsyncWrapQueueDestroyId(handle->env, handle->async_id);
  handle->async_id = -1;
}
...
napi_value CompressionCtor(napi_env env,
  ...
  handle->async_id = UbiAsyncWrapNextId(env);
  ...
  UbiAsyncWrapEmitInit(env,
                       ...
                       UbiAsyncWrapExecutionAsyncId(env),
                       self);
  return self;
}
...
napi_value CompressionAsyncReset(napi_env env, napi_callback_info info) {
  ...
  if (handle == nullptr) return Undefined(env);
  UbiAsyncWrapReset(env, &handle->async_id);
  UbiAsyncWrapEmitInit(
      env, handle->async_id, handle->provider_type, UbiAsyncWrapExecutionAsyncId(env), self);
  return Undefined(env);
}
