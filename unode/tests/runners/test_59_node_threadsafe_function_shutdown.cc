...
class Test59NodeThreadsafeFunctionShutdown : public FixtureTestBase {};

TEST_F(Test59NodeThreadsafeFunctionShutdown, PortedCoreFlow) {
  ...
  ASSERT_EQ(napi_set_named_property(s.env, global, "__ttsfs", module_value), napi_ok);
...
if (typeof __ttsfs !== 'function') throw new Error('exportIsFunction');
)JS"));
}
