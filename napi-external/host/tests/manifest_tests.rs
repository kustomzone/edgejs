...
use napi_host::run_wasix_main_capture_stdout;

fn project_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("host crate must have workspace root parent")
        .to_path_buf()
}
...
fn build_native_test(name: &str) -> PathBuf {
    ...
    let status = Command::new("./guest/build-test-native.sh")
        .arg(name)
        .current_dir(&root)
        .status()
        .expect("failed to execute guest/build-test-native.sh");
...
fn build_wasix_test(name: &str) -> PathBuf {
    let root = project_root();
    let status = Command::new("./guest/build-test-wasix.sh")
        .arg(name)
        .current_dir(&root)
        .status()
        .expect("failed to execute guest/build-test-wasix.sh");
...
fn manifest_tests_match_native_and_wasix() {
    let root = project_root();
    let manifest_path = root.join("guest/tests/manifest.json");
    let manifest_content =
        std::fs::read_to_string(&manifest_path).expect("failed to read guest/tests/manifest.json");
