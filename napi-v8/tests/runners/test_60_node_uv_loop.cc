...
class Test60NodeUvLoop : public FixtureTestBase {};

TEST_F(Test60NodeUvLoop, PortedCoreFlow) {
  ...
  ASSERT_EQ(napi_set_named_property(s.env, global, "__tuv", exports), napi_ok);
...
globalThis.__uvCalled = false;
__tuv.SetImmediate(() => { globalThis.__uvCalled = true; });
)JS"));
  ...
  ASSERT_TRUE(run_js(R"JS(
if (!globalThis.__uvCalled) throw new Error('setImmediate');
)JS"));
}
