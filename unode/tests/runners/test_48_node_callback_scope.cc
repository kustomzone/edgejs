...
class Test48NodeCallbackScope : public FixtureTestBase {};

TEST_F(Test48NodeCallbackScope, PortedCoreFlow) {
  ...
  ASSERT_EQ(napi_set_named_property(s.env, global, "__tcs", exports), napi_ok);
...
if (__tcs.runInCallbackScope({}, 'test-resource', () => 42) !== 42) {
  throw new Error('runInCallbackScope');
}
globalThis.__resolved = false;
__tcs.testResolveAsync().then(() => { globalThis.__resolved = true; });
)JS"));
  ...
  ASSERT_TRUE(run_js(R"JS(
if (!globalThis.__resolved) throw new Error('resolveAsync');
)JS"));
}
