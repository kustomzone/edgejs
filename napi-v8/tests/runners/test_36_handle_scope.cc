...
TEST_F(Test36HandleScope, PortedCoreFlow) {
  ...
  ASSERT_EQ(napi_set_named_property(s.env, global, "__ths", exports), napi_ok);
...
__ths.NewScope();
if (!(__ths.NewScopeEscape() instanceof Object)) throw new Error('escapeObject');
__ths.NewScopeEscapeTwice();
let threw = false;
try {
  __ths.NewScopeWithException(() => { throw new RangeError(); });
} catch (e) {
  threw = e instanceof RangeError;
}
if (!threw) throw new Error('scopeException');
)JS"));
}
