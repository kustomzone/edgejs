...
if [[ $# -ne 1 ]]; then
  ...
  echo "test-name maps to guest/tests/<test-name>.c" >&2
  exit 1
fi
...
TEST_SRC="$ROOT_DIR/guest/tests/${TEST_NAME}.c"
NAPI_INCLUDE_DIR="$ROOT_DIR/guest/napi-headers"
TEST_INCLUDE_DIR="$ROOT_DIR/guest/tests"
NATIVE_INIT_SRC="$ROOT_DIR/guest/napi_native_init.cc"
